...
@shared_task
def xfer_to_open_humans(fitbit_data, oh_id, num_submit=0, **kwargs):
    """
    ...
    try:
        add_data_to_open_humans(fitbit_data, oh_member, tempdir)
    finally:
...
def add_data_to_open_humans(fitbit_data, oh_member, tempdir):
    """
    ...
    """
    # Create data file.
    data_filepath, data_metadata = make_datafile(fitbit_data, tempdir)
...
def make_datafile(fitbit_data, tempdir):
    """
    Make a Fitbit data file in the tempdir.
    """
    filename = 'fitbit_data_' + datetime.today().strftime('%Y%m%d')
    filepath = os.path.join(tempdir, filename)

    with open(filepath, 'w') as f:
        f.write(fitbit_data)
    metadata = {
        'tags': ['fitbit', 'health', 'measure'],
